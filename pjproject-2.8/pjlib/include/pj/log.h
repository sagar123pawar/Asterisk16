...
#define pj_log_wrapper_fmt_string(arg) pj_fmt_string arg
#define pj_log_wrapper_get_src_strings(arg) pj_src_string arg

static inline char *pj_fmt_string(const char *src, const char *format, ...)
{
...
static inline char *pj_src_string(const char *src, const char *format, ...)
{
	if (src && strlen(src) > 0) {
		return strdup(src);
	}
...
#define __pj_log_wapper(level, file, line, src, strings) do {														\
	char *file_name = (char *)strrchr((file), '/');																	\
	if (src && !strcasestr(file, src)) {																			\
		pj_log_wrapper_##level(("", "[%s] [%s:%d] %s", src, file_name ? ++file_name : (file), (line), (strings)));	\
		free(src);																									\
		src = NULL;																									\
	} else {																										\
		pj_log_wrapper_##level(("", "[%s:%d] %s", file_name ? ++file_name : (file), (line), (strings)));			\
	}																												\
} while (0)

#define PJ_LOG(level, arg) do {											\
	char *log_str = pj_log_wrapper_fmt_string(arg);						\
	char *src_str = pj_log_wrapper_get_src_strings(arg);				\
	if (log_str) {														\
		__pj_log_wapper(level, __FILE__, __LINE__, src_str, log_str);	\
		free(log_str);													\
		log_str = NULL;													\
	}																	\
} while (0)
