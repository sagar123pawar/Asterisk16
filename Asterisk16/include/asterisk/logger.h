...
#define ast_log(level, ...) do {                                                               \
	__ast_log(level, __FILE__, __LINE__, __PRETTY_FUNCTION__, AST_MODULES_INDEX, __VA_ARGS__); \
} while (0)

void __ast_log_ap(int level, const char *file, int line, const char *function, int index, const char *fmt, va_list ap)
	 __attribute__((format(printf, 6, 0)));

#define ast_log_ap(level, fmt, ap) do {                                                            \
	__ast_log_ap(level, __FILE__, __LINE__, __PRETTY_FUNCTION__, AST_MODULES_INDEX, fmt, ap); \
} while (0)
...
#define ast_log_safe(level, ...) do {                                                                     \
	__ast_log_safe(level, __FILE__, __LINE__, __PRETTY_FUNCTION__, AST_MODULES_INDEX, __VA_ARGS__); \
} while (0)
...
#define ast_log_callid(level, callid, ...) do {                                                                      \
	__ast_log_callid(level, __FILE__, __LINE__, __PRETTY_FUNCTION__, AST_MODULES_INDEX, callid, __VA_ARGS__); \
} while (0)
 ...
 */
#ifdef GRANDSTREAM_NETWORKS
void __attribute__((format(printf, 6, 7))) __ast_verbose(const char *file, int line, const char *func, int level, int index, const char *fmt, ...);
#else
 ...
 */
#ifdef GRANDSTREAM_NETWORKS
void __attribute__((format(printf, 7, 8))) __ast_verbose_callid(const char *file, int line, const char *func, int level, int index, ast_callid callid, const char *fmt, ...);

#define ast_verbose(...) __ast_verbose(__FILE__, __LINE__, __PRETTY_FUNCTION__, -1, AST_MODULES_INDEX, __VA_ARGS__)
#define ast_verbose_callid(callid, ...) __ast_verbose_callid(__FILE__, __LINE__, __PRETTY_FUNCTION__, -1, AST_MODULES_INDEX, callid, __VA_ARGS__)

void __attribute__((format(printf, 7, 0))) __ast_verbose_ap(const char *file, int line, const char *func, int level, int index, ast_callid callid, const char *fmt, va_list ap);
...
#endif
#ifdef GRANDSTREAM_NETWORKS
#define AST_LOG_NOTICE     __LOG_NOTICE
#else
...
#endif
#ifdef GRANDSTREAM_NETWORKS
#define AST_LOG_WARNING    __LOG_WARNING
#else
...
#endif
#ifdef GRANDSTREAM_NETWORKS
#define AST_LOG_ERROR      __LOG_ERROR
#else
...
#ifdef GRANDSTREAM_NETWORKS
#define LOG_VERBOSE    __LOG_VERBOSE
#else
...
#endif
#ifdef GRANDSTREAM_NETWORKS
#define AST_LOG_VERBOSE    __LOG_VERBOSE
#else
...
#endif
#ifdef GRANDSTREAM_NETWORKS
#define AST_LOG_DTMF    __LOG_DTMF
#else
 ...
 */
#ifdef GRANDSTREAM_NETWORKS
#define ast_log_dynamic_level(level, ...) __ast_log(level, __FILE__, __LINE__, __PRETTY_FUNCTION__, AST_MODULES_INDEX, __VA_ARGS__)
#else
...
#define ast_verb(level, ...) \
	do { \
		if (VERBOSITY_ATLEAST(level) ) { \
			__ast_verbose(__FILE__, __LINE__, __PRETTY_FUNCTION__, level, AST_MODULES_INDEX, __VA_ARGS__); \
		} \
	} while (0)
...
#define ast_verb_callid(level, callid, ...) \
	do { \
		if (VERBOSITY_ATLEAST(level) ) { \
			__ast_verbose_callid(__FILE__, __LINE__, __PRETTY_FUNCTION__, level, AST_MODULES_INDEX, callid, __VA_ARGS__); \
		} \
	} while (0)
...
enum ast_log_enum {
	...
	AST_NOTICE_LOG_ENUM  = 1 << __LOG_NOTICE,
	AST_WARNING_LOG_ENUM = 1 << __LOG_WARNING,
	AST_ERROR_LOG_ENUM   = 1 << __LOG_ERROR,
	AST_VERBOSE_LOG_ENUM = 1 << __LOG_VERBOSE,
	AST_DTMF_LOGO_ENUM   = 1 << __LOG_DTMF,
...
#ifndef AST_MODULES_INDEX
static int ast_module_index AST_MAYBE_UNUSED;
