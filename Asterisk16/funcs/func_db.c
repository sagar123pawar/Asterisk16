...
/*** DOCUMENTATION
	<function name="UCMDB" language="en_US">
		<synopsis>
	...
	<function name="UCMDB_EXISTS" language="en_US">
		<synopsis>
...
#ifdef GRANDSTREAM_NETWORKS
static int function_ucm_db_read(struct ast_channel *chan, const char *cmd,
			    char *parse, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(parse)) {
		ast_log(LOG_WARNING, "UCMDB requires an argument, UCMDB(<family>/<key>)\n");
		return -1;
	}
	...
	if (ast_ucm_db_get(args.family, args.key, buf, len - 1)) {
		ast_debug(1, "UCMDB: %s/%s not found in database.\n", args.family, args.key);
	} else {
...
static int function_ucm_db_write(struct ast_channel *chan, const char *cmd, char *parse,
			     const char *value)
{
	AST_DECLARE_APP_ARGS(args,
		AST_APP_ARG(family);
	...
	if (ast_strlen_zero(parse)) {
		ast_log(LOG_WARNING, "UCMDB requires an argument, UCMDB(<family>/<key>)=<value>\n");
		return -1;
	}
	...
	if (ast_ucm_db_put(args.family, args.key, value,1)) {
		ast_log(LOG_WARNING, "UCMDB: Error writing value to database.\n");
	}

	return 0;
}

static struct ast_custom_function ucm_db_function = {
	.name = "UCMDB",
	.read = function_ucm_db_read,
	.write = function_ucm_db_write,
};

static int function_ucm_db_exists(struct ast_channel *chan, const char *cmd,
			      char *parse, char *buf, size_t len)
{
	AST_DECLARE_APP_ARGS(args,
		AST_APP_ARG(family);
	...
	if (ast_strlen_zero(parse)) {
		ast_log(LOG_WARNING, "UCMDB_EXISTS requires an argument, UCMDB(<family>/<key>)\n");
		return -1;
	}
...
static struct ast_custom_function ucm_db_exists_function = {
	.name = "UCMDB_EXISTS",
	.read = function_ucm_db_exists,
	.read_max = 2,
};
...
#ifdef GRANDSTREAM_NETWORKS
	res |= ast_custom_function_unregister(&ucm_db_function);
	res |= ast_custom_function_unregister(&ucm_db_exists_function);
#endif
...
#ifdef GRANDSTREAM_NETWORKS
	res |= ast_custom_function_register_escalating(&ucm_db_function, AST_CFE_BOTH);
	res |= ast_custom_function_register(&ucm_db_exists_function);
#endif
